...
export function TweetContextProvider({ children }) {
  ...
  useEffect(() => {
    async function getTweetList() {
      ...
      const tweetList = docs.map((doc) =>
        doc.data().forEach((tweet) => console.log("tweet", tweet.uid))
      );
      const filteredTweetList = tweetList.forEach((tweet) =>
        console.log("tweet", tweet.uid)
      );
