...
function reducer(tweets, action) {
  switch (action.type) {
    case ACTIONS.ADD_TWEET: 
      return console.log("tweets", tweets)
...
export function TweetContextProvider({children}) {
  ...
  const [tweets, dispatch] = useReducer(reducer, posts)
  console.log("tweets", postCollection)
  ...
  async function sendUserTweet(tweetMessage) {
    ...
    dispatch({ type: ACTIONS.ADD_TWEET, payload: {avatar:
      "https://placekitten.com/200/287",
      date: date,
      text: tweetMessage,
