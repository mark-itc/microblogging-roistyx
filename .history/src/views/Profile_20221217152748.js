...
export function Profile() {
  ...
  const { picUrl, setPicUrl, newCurrentUser, 
    ...
    useEffect(() => {
      const storageRef = ref(storage, `${currentUser.uid}/profile-pic`);
      // console.log(`${storageRef.bucket}/${currentUser.uid}/profile-pic`)
 ...
 console.log("newCurrentUser",newCurrentUser)
  return (
    <>
      ...
      {currentUser ? <div><img alt="" style={{ width: "50px" }} src={storageRef} /><p>{currentUser.uid}, {currentUser.email}</p></div> : "No user logged in" }
