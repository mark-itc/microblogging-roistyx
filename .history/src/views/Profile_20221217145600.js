...
export function Profile() {
  ...
  const { picUrl, setPicUrl, newCurrentUser, 
    ...
    useEffect(() => {
      // const myStorage = storage.storage();
      // const fileRef = myStorage.ref(`${currentUser.uid}/profile-pic`)
 ...
 console.log("newCurrentUser",newCurrentUser)
  return (
    <>
      ...
      {currentUser ? <p>{currentUser.uid}, {currentUser.email}</p> : "No user logged in" }
      <img alt="" style={{ width: "50px" }} src={`https://firebasestorage.googleapis.com/v0/b/myauth-development-264bb.appspot.com/o/${currentUser.uid}%2Fprofile-pic?alt=media&token=fb391386-1ca2-436b-a3ad-2fafee70fe43`} />
      <h3>{progress}</h3>
    </>
  );
}
